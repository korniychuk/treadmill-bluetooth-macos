...
| # | Действие | Acceptance |
|---|---|---|
| 0.1 | HR silence: `sleep_until(last_hr_at + HR_NOTIFICATION_TIMEOUT)` как у treadmill (031) | unit test: `tokio::time::pause` **+ соседний быстрый arm** (без sibling-arm тест невалиден) |
| 0.2 | Убрать `unwrap_or(0.0)` на **трёх engage-путях** speed (~755, ~771, ~797); `None` → skip tick | MORE_DATA без speed не стартует Ramp с 0; unit/regression |
| 0.3 | Atomic config write: temp + rename в `zone_hold` / `goals` TOML writers | crash mid-write не оставляет empty/truncated config |
| 0.4 | **`tm doctor`**: матрица живости live — возраст last 0x2ACD, возраст last HR frame, contact, streaming flag, `zh_phase` vs `enabled` | один CLI-вызов заменяет 4 log sources; MTTR↓ |
| 0.5 | Document liveness matrix (§3.2) in `CLAUDE.md` as invariant | reviewers check new code against matrix |
| 0.6 | Karvonen: `WARN` + explicit fallback to HrMax when `resting_hr` missing | no silent method switch without log |
| 0.7 | Smoke: power-off treadmill with HR worn; remove H10; `tm zone off` mid-ramp | already fixed paths stay green |
