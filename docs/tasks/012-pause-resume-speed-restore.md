...
## Доработка: «крейсерская» скорость вместо хвоста торможения

Первая версия брала `last_walking_speed` — последний ненулевой сэмпл перед
паузой. Но дорожка **сама** плавно тормозит перед паузой; замер логов W2 Pro:
`2.5 → 1.86 → 1.07 → 0.59 → 0` за **~2–3 с**. Поэтому захватывалось `~0.6`
(хвост), и restore был бессмысленным (`0.5 → 0.6`).

Фикс: демон держит короткий буфер `(Instant, speed)` (`SPEED_HISTORY_RETENTION`
= 45 с) и на паузе оценивает крейсерскую скорость `cruising_speed()`:

- отбрасываем сэмплы за последние `SPEED_CRUISE_DECEL_SKIP` = 10 с (хвост
  торможения; замер ~3 с, запас) и медленнее `SPEED_CRUISE_FLOOR_KMH` = 0.8
  (рампа/минимум);
- берём **медиану** оставшихся «ходовых» сэмплов;
- fallback для слишком короткой сессии — **пик** ходовой скорости (лента тормозит
  только вниз, так что максимум = крейсер), затем `last_walking_speed`.

Pure + 4 юнит-теста. Так `0.5 → 2.5` восстанавливается корректно.
