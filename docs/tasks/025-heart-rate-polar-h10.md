...
Дефолтный формат HR в `stats` беру как **`♥ avg/max`** (trimmed avg / p95). Если
хочешь по умолчанию тройку-диапазон (`♥ low–avg–high`, твоя «от и до» идея) — это
меняется одной строкой форматтера; скажи до реализации M2. Полный разрез (top/
bottom-10%, n) закладываю опциональным `--hr`, а не в дефолт.
