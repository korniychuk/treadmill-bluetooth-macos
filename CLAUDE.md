...
репо). Правки **подхватываются на лету без рестарта** (задача 017): демон следит
за mtime `goals.json` и перечитывает цели при изменении (≤5 с, только когда файл
реально менялся). `workout_gap_minutes` и так read-time (ретроактивен). Мгновенно
