...
- `src/zone_hold.rs` — **Zone Hold** (задача 027): HR-адаптивная подстройка
  ...
  в `daemon_status`. CLI `tm zone on/off/setup/limits/target/list/mode` (без
  аргумента — статус); `on`/`setup` — интерактивный онбординг возраста;
  `list` — таблица всех зон (id, bpm-диапазон, max_speed) с меткой текущего
  таргета, чтобы `target` не был гаданием.
...
cargo run -- zone      # Zone Hold: статус (без аргумента) или on/off/setup/limits/target/list/mode (docs/tasks/027)
