...
When to delegate is in the global instructions, section "Coding-Agent Executor"; how (brief, flags, exit codes, review, integration, Grok fallback) is the `executor` skill — load it before launching a wrapper (`~/.claude/skills/executor/SKILL.md`). This section only adds what is specific to this repository. Follow the branch that matches who you are.

**If you are a sandboxed subagent** (`CODEX_AGENT_DEPTH` or `GROK_AGENT_DEPTH` is set; cwd is a `…_codex-<name>` / `…_grok-<name>` worktree, or the main repo in read-only research mode): do the assigned task directly and completely, then commit on your branch (research mode: just report). Never call `codex-agent.sh` / `grok-agent.sh`. The rest of this section is not for you.

**If you are the orchestrating session:**
- **Codex-executor delegation is the DEFAULT in this repo.** This overrides the global explicit-opt-in rule: no ask is needed. Apply the global criterion (expected effort, not diff size) before the first edit, then load the `executor` skill and launch; if work that meets it stays in the main thread, say why in one line.
- **Keep in the main thread here:** anything that needs the real treadmill, HR sensor or BLE radio (`cargo run -- connect|daemon|hr|zone|…`), the LaunchAgent (`scripts/install-daemon.sh`, `scripts/uninstall-daemon.sh`) and releases (`scripts/release.sh` pushes a tag).
- **Gates a brief must name:** `cargo test`, `cargo clippy`.
- **Stack:** Rust — the cargo net preset is automatic.
